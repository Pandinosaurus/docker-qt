# Docker container to build Qt 6.11 for Android projects with latest cmake
# Image: a12e/docker-qt:6.11-android

FROM ubuntu:24.04
LABEL org.opencontainers.image.authors="Aurélien Brooke <dev@abrooke.fr>"
...
&&  curl -Lo tools.zip https://dl.google.com/android/repository/commandlinetools-linux-13114758_latest.zip \
...
&&  pip install --break-system-packages aqtinstall==${AQTINSTALL_VERSION} \
...
&&  for QT_ARCH in ${QT_ARCHS} ; do \
        ...
        ./Configure android-${OPENSSL_ARCH} shared zlib-dynamic -no-engine no-tests --prefix=/opt/qt/${QT_VERSION}/android_${QT_ARCH} -D__ANDROID_API__=28 -Wl,-z,max-page-size=16384 -Wl,-z,common-page-size=16384 ; \
        make build_libs ; \
        make install_sw ; \
        cd ../ ; \
